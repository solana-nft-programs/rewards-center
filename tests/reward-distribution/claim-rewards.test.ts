...
test("Claim rewards", async () => {
  ...
  const rewardDistributorAtaBefore = await getAccount(
    provider.connection,
    getAssociatedTokenAddressSync(rewardMintId, rewardDistributorId, true)
  );
  const rewardDistributorBefore = Number(rewardDistributorAtaBefore.amount);

  console.log(
    Number(userPaymentAtaBefore.amount),
    rewardDistributorId.toString(),
    rewardDistributorBefore
  );
  await executeTransactions(
    provider.connection,
    await claimRewards(
