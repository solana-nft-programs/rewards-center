...
declare_id!("rwcn6Ry17ChPXpJCN2hoK5kwpgFarQqzycXwVJ3om7U");
...
pub mod cardinal_rewards_center {
    ...
    //// stake_entry::pnfts ////
    pub fn stake_pnfts<'key, 'accounts, 'remaining, 'info>(ctx: Context<'key, 'accounts, 'remaining, 'info, StakePNFTCtx<'info>>) -> Result<()> {
        stake_entry::pnfts::stake_pnft::handler(ctx)
    }
    pub fn unstake_pnfts<'key, 'accounts, 'remaining, 'info>(ctx: Context<'key, 'accounts, 'remaining, 'info, UnstakePNFTCtx<'info>>) -> Result<()> {
