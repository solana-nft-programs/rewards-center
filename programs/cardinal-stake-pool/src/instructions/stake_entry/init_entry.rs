...
pub fn handler(ctx: Context<InitEntryCtx>, _user: Pubkey) -> Result<()> {
    ...
    if !stake_pool.requires_creators.is_empty() || !stake_pool.requires_collections.is_empty() || stake_pool.requires_authorization {
        ...
        if !ctx.accounts.stake_mint_metadata.data_is_empty() {
            let mint_metadata_data = ctx.accounts.stake_mint_metadata.try_borrow_mut_data().expect("Failed to borrow data");
            if ctx.accounts.stake_mint_metadata.to_account_info().owner.key() != mpl_token_metadata::id() {
            ...
            if stake_mint_metadata.mint != ctx.accounts.stake_mint.key() {
        ...
        if stake_pool.requires_authorization && !allowed {
            ...
            if stake_entry_authorization_account.pool == stake_entry.pool && stake_entry_authorization_account.mint == stake_entry.stake_mint {
                allowed = true;
            }
        }
        if !allowed {
